...
public class DresserScanner : IDisposable
{
    ...
    {
        Dispeller.Framework.Update += OnFrameworkUpdate;
    }
    ...
    private unsafe void OnFrameworkUpdate(IFramework framework)
    {
        try
        {
            ...
            lock (LockObject)
            {
                ...
                if (itemCount > 0)
                {
                    Dispeller.Log.Information($"OnFrameworkUpdate: Cached {itemCount} items from dresser (cache was empty: {wasEmpty})");
                }
            }
        }
        catch
    ...
    public static unsafe bool TryRefresh()
    {
        try
        {
            ...
            if (agent == null)
            {
                Dispeller.Log.Debug("TryRefresh: AgentMiragePrismPrismBox.Instance() returned null - dresser not open");
                return false;
            }

            if (!agent->IsAddonReady())
            {
                Dispeller.Log.Debug("TryRefresh: Agent is not ready (IsAddonReady = false) - dresser not open");
                return false;
            }

            if (agent->Data == null)
            {
                Dispeller.Log.Debug("TryRefresh: Agent data is null - dresser not initialized");
                return false;
            }
            ...
            lock (LockObject)
            {
                ...
                Dispeller.Log.Information($"TryRefresh: Loaded {itemCount} items from dresser");
                return true;
            }
        }
        catch (Exception ex)
        {
            Dispeller.Log.Error(ex, "Error in TryRefresh");
            return false;
        }
    }
    ...
    public static bool HasCachedData()
    {
        lock (LockObject)
        {
            ...
            if (hasData)
            {
                Dispeller.Log.Debug($"HasCachedData: Cache contains {_cachedDresserItems.Count} items");
            }
            return hasData;
        }
    }
