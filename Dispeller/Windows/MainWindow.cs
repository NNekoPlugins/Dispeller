...
{
    private readonly Dispeller plugin;
    ...
    public MainWindow(Dispeller plugin)
    ...
    {
        var icon = Dispeller.TextureProvider.GetFromGameIcon(new Dalamud.Interface.Textures.GameIconLookup(id)).GetWrapOrDefault();
        return icon;
    }
    ...
    private void ScanDresser()
    {
        ...
        try
        {
            ...
            Dispeller.Log.Information($"ScanDresser: Starting scan - cached items before refresh: {cachedCountBefore}");
            ...
            Dispeller.Log.Information($"Dresser scan: Found {dresserItems.Count} items (cached before: {cachedCountBefore}, cached after: {cachedCountAfter}, refreshed: {refreshed})");
            ...
            if (dresserItems.Count == 0)
            {
                if (!hasCachedData && !refreshed)
                {
                    ...
                    Dispeller.Log.Warning("Dresser scan: No cached data and dresser not open");
                }
                else if (hasCachedData && !refreshed)
                {
                    ...
                    Dispeller.Log.Warning($"Dresser scan: Had {cachedCountBefore} cached items but got 0 after refresh attempt");
                }
                else
                {
                    statusMessage = "Your glamour dresser appears to be empty!";
                    Dispeller.Log.Warning("Dresser scan: Dresser is open but empty");
                }
                sharedGroups = null;
                return;
            }
        ...
        catch (Exception ex)
        {
            ...
            sharedGroups = null;
            Dispeller.Log.Error(ex, "Error during dresser scan");
        }
        finally
        {
            isScanning = false;
    ...
    {
        var sheet = Dispeller.DataManager.GetExcelSheet<Item>()!;
    ...
    private bool CanGoInArmoire(uint itemId)
    {
        ...
        var cabinetSheet = Dispeller.DataManager.GetExcelSheet<Cabinet>()!;
